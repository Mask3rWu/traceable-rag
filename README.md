...
在 47 篇文档、3779 个 chunk、2083 道问题上（Dense/BM25 各召回 50 条，等权 RRF，`rank_constant=60`，`eval/v2`）：
...
> 早期 `eval/v1` 的批量题字面泄漏明显、偏向词法检索，BM25 单路排序偏高；等权 RRF 的优势主要体现在更深的候选集（Recall@20/50 最高），适合为后续 Reranker 提供覆盖。当前为中性基线，融合权重待独立评测集就绪后再定。
