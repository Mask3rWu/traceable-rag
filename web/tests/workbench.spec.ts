...
test('chapter research exposes evidence contribution and inference', async ({ page }) => {
  ...
  const evidence = {
    ...
    quote_truncated: false, visual_assets: [], retrieval: [{ query: '毁伤等级分类', final_rank: 1, dense_rank: 1, dense_score: 0.9, bm25_rank: 2, bm25_score: 8, fusion_score: 0.03 }], verified: true,
  }
  ...
  const packet = (chapter_id: string, title: string, text: string) => ({
    ...
    claims: [{ claim_id: `C-${chapter_id}`, text, conclusion_type: 'synthesized', citations: [{ evidence_id: 'ev-levels', quote: '完好、受限、丧失和彻底毁坏' }], citation_verified: true }],
  ...
  await expect(page.getByText('采用四级毁伤等级', { exact: true })).toBeVisible()
  await expect(page.getByText(/四种可区分状态可作为四级分级/)).toBeVisible()
  await page.getByRole('button', { name: /二、运动能力评估标准/ }).click()
  await expect(page.getByText('运动能力按统一四级体系判定。')).toBeVisible()
  await page.locator('.contribution-item').click()
  await expect(page.locator('.evidence-detail blockquote')).toContainText('完好、受限、丧失和彻底毁坏')
